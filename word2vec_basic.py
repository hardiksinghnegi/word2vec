...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  ===============================================================================
  """
  ''' Set window size to work on'''
  window_size = 2*skip_window+1
  batch_list = list()
  size_flag = False

  '''Continue till the samples collected are less than batch size'''
  while not size_flag:
    '''Work on the window size list'''
    win_grab = data[data_index:data_index+window_size]
    context_index = len(win_grab)//2
    left = context_index - 1
    right = context_index + 1
    samp_index = 0
    while(samp_index<num_skips):
      if len(batch_list)<batch_size:
        batch_list.append((win_grab[context_index], win_grab[left]))
        left -= 1
        samp_index += 1
      if len(batch_list)<batch_size:
        batch_list.append((win_grab[context_index], win_grab[right]))
        right += 1
        samp_index += 1
      else:
        size_flag = True
  ...
  '''Add context word in batch_id and predicted word in pred_id'''
  for val in batch_list:
    batch_id.append(val[0])
    pred_id.append(val[1])

  ''' Enter the required context values in batch and predicted values in labels'''
  for index in range(len(batch_id)):
    batch[index] = batch_id[index]
    labels[index][0] = pred_id[index]
